...
public class MockControllerTest extends AbstractMock {
	...
	private void callProxyIsPaidFalse() throws Exception {
		// when
		mockGetPaymentInfo();
		MockHttpServletResponse response = mvc.perform(get("/api/v1/payment/check/ABC").accept(MediaType.APPLICATION_JSON))
				.andReturn().getResponse();
		// then
		assertThat(response.getStatus()).isEqualTo(HttpStatus.OK.value());
	}
	
	private void callProxyIsPaidTrue() throws Exception {
		// when
